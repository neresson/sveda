...
const fromDocument = (document) => {
    ...
    return {
        ...
        web_enabled: Boolean(web.enabled ?? true),
        web_searxng_url: String(web.searxng_url ?? ''),
    };
};
    ...
    try {
        const response = await fetch(props.saveUrl, {
            ...
            body: JSON.stringify({
                ...
                web: {
                    enabled: Boolean(form.web_enabled),
                    searxng_url: form.web_searxng_url.trim(),
                },
            }),
        });
...
<template>
    <div class="contents">
        ...
        <form class="flex flex-col gap-4 border border-ink p-4 lg:p-6" @submit.prevent="save">
            ...
            <div class="grid gap-4 border-t border-grid pt-4 md:grid-cols-2">
                <label class="flex items-center gap-2 font-mono text-xs md:col-span-2">
                    <input v-model="form.web_enabled" name="web_enabled" type="checkbox" class="size-3.5 border border-ink accent-ink">
                    {{ t('runtime.web') }} · {{ t('runtime.web_enabled') }}
                </label>
                <p class="font-mono text-[11px] text-muted md:col-span-2">{{ t('runtime.web_hint') }}</p>
                <label class="flex flex-col gap-2 md:col-span-2">
                    <span class="font-mono text-[11px] tracking-[0.14em] text-muted">{{ t('runtime.web_searxng') }}</span>
                    <input
                        v-model="form.web_searxng_url"
                        name="web_searxng_url"
                        class="border border-ink bg-canvas px-3.5 py-3 font-mono text-sm outline-none"
                    >
                    <span class="font-mono text-[11px] text-muted">{{ t('runtime.web_searxng_hint') }}</span>
                </label>
            </div>
