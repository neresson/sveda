...
provide('reloadAdmin', (sync = {}) => {
    const nextPage = typeof sync.page === 'string' ? sync.page.trim() : '';
    const nextUrl = typeof sync.url === 'string' && sync.url.trim() !== ''
        ? sync.url.trim()
        : nextPage && typeof payload.urls?.[nextPage] === 'string'
            ? payload.urls[nextPage]
            : '';
    const target = nextUrl || window.location.href;
    const current = new URL(window.location.href);
    let next;
    try {
        next = new URL(target, window.location.origin);
    } catch {
        return visit(window.location.href, { historyMode: 'replace' });
    }

    const samePlace = next.pathname === current.pathname && next.search === current.search;
    return visit(next.href, { historyMode: samePlace ? 'replace' : 'push' });
});
