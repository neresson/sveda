...
use veda_protocol::{
    load_sidecar_contract, parse_sse_line, SidecarContract, ACCEPT_VEDA_STREAM, PROTOCOL_VERSION,
    SSE_DONE_LINE, STREAM_EVENTS, TOKEN_PREFIX,
};
use veda_server::{app, AppState, Config};
use veda_store::Checkpoint;
...
async fn embed_token_requires_host_key_when_configured() {
    ...
    headers.insert("x-veda-host-key", "sidecar-host-secret".parse().unwrap());
    let (status, _, _) = send(
        state.clone(),
        "POST",
        "/veda/embed/token",
...
async fn stream_hello_world_matches_sidecar_contract() {
    ...
    let mut headers = json_headers();
    headers.insert("accept", ACCEPT_VEDA_STREAM.parse().unwrap());
    headers.insert("x-veda-embed-token", token.parse().unwrap());
    ...
    assert_eq!(
        response_headers
            .get(HeaderName::from_static("x-veda-protocol-version"))
            .unwrap()
            .to_str()
            .unwrap(),
        PROTOCOL_VERSION
...
async fn message_and_histories_match_sidecar_contract() {
    ...
    assert_eq!(payload["explanation"], "Hello from Veda");
    ...
    let (status, _, body) = send(
        state.clone(),
        "GET",
        "/veda/chat-histories/chat-contract-message",
        headers.clone(),
        Body::empty(),
    )
    .await;
...
async fn histories_are_scoped_to_embed_visitor() {
    ...
    let (status, _, _) = send(
        state,
        "GET",
        "/veda/chat-histories/chat-secret",
        auth_headers(&token_b),
        Body::empty(),
    )
    .await;
...
async fn cors_preflight_allows_contract_headers() {
    ...
    headers.insert(
        ...
        "content-type,x-veda-embed-token,x-veda-protocol"
            .parse()
            .unwrap(),
    );
    ...
    let allow_headers = response_headers
        ...
        .unwrap()
        .to_ascii_lowercase();
    assert!(allow_headers.contains("x-veda-embed-token"));
    assert!(allow_headers.contains("x-veda-protocol"));
}
