...
use veda_protocol::{HEADER_ADMIN_KEY, HEADER_EMBED_TOKEN, TOKEN_PREFIX};
...
fn admin_headers() -> HeaderMap {
    ...
    headers.insert(HEADER_ADMIN_KEY, "veda-admin-secret".parse().unwrap());
    headers
}
...
async fn reads_settings_with_bearer_admin_key() {
    let mut headers = HeaderMap::new();
    headers.insert("authorization", "Bearer veda-admin-secret".parse().unwrap());
    let (status, _) = send(
        admin_state(),
        "GET",
        "/veda/admin/settings",
...
async fn public_embed_config_omits_secrets() {
    ...
    let (status, body) = send(state, "GET", "/veda/embed/config", headers, Body::empty()).await;
    assert_eq!(status, StatusCode::OK);
    let payload: Value = serde_json::from_slice(&body).unwrap();
    let text = String::from_utf8(body).unwrap();
    assert_eq!(payload["welcome_message"], "Hi from Veda");
    assert_eq!(payload["default_model"], "deepseek-v4-flash-responses");
