...
use veda_llm::ChatMessage;
use veda_mcp::{HostMcpClient, McpCallContext, McpCredentials};
use veda_protocol::{StreamEvent, StreamRequest};
...
pub async fn run_turn(
    ...
    let stored = state.store.record(&visitor_id, &chat_id);
    if history.is_empty() {
    ...
    let client_tool_specs = veda_agent::client_tool_specs(request.client_tools.as_ref());
    let agent_request = AgentRequest {
        prompt: prompt.clone(),
        history,
        chat_id: Some(chat_id.clone()),
        model: request.model.clone(),
        thinking: veda_agent::thinking_enabled(request.options.as_ref()),
    ...
    let events = match veda_agent::run_with_tools(
        state.llm.clone(),
        catalog.clone(),
        AgentConfig {
...
fn finalize_turn(
    ...
    stream! {
        ...
        let runtime_config = state.runtime_config();
        let catalog = state.catalog();
        let existing = state.store.record(&visitor_id, &chat_id);
        ...
        if should_compact(
            ...
            tokio::spawn(async move {
                ...
                if !summary.is_empty() {
                    store.set_summary(&visitor_id, &chat_id, summary);
                }
            });
        }
    }
