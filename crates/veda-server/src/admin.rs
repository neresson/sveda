...
fn header_admin_key(headers: &HeaderMap) -> Option<String> {
    if let Some(value) = headers
        .get(veda_protocol::HEADER_ADMIN_KEY)
    ...
    headers
        ...
        .map(str::trim)
        .filter(|token| !token.is_empty() && !token.starts_with(veda_protocol::TOKEN_PREFIX))
        .map(ToOwned::to_owned)
}
...
pub async fn update_settings(
    ...
    let stored = state.settings.replace(merged);
...
fn apply_runtime(state: &AppState, document: &crate::settings::SettingsDocument) {
