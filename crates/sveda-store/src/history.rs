...
impl HistoryStore {
    pub fn memory() -> Self {
        Self::Memory(MemoryHistoryStore::new())
    }
    ...
    pub async fn ping(&self) -> Result<(), StoreError> {
        match self {
            Self::Memory(_) => Ok(()),
            Self::Postgres(postgres) => postgres.ping().await,
        }
    }

    pub async fn checkpoint(&self, input: Checkpoint) -> Result<HistoryRecord, StoreError> {
        match self {
            Self::Memory(store) => Ok(store.checkpoint(input)),
            Self::Postgres(store) => store.checkpoint(input).await,
        }
    }

    pub async fn list(&self, visitor_id: &str) -> Result<Vec<HistorySummary>, StoreError> {
        match self {
            Self::Memory(store) => Ok(store.list(visitor_id)),
            Self::Postgres(store) => store.list(visitor_id).await,
        }
    }
    ...
    pub async fn get(
        ...
        match self {
            Self::Memory(store) => Ok(store.get(visitor_id, chat_id)),
            Self::Postgres(store) => store.get(visitor_id, chat_id).await,
        }
    }
    ...
    pub async fn record(
        ...
        match self {
            Self::Memory(store) => Ok(store.record(visitor_id, chat_id)),
            Self::Postgres(store) => store.record(visitor_id, chat_id).await,
        }
    }
    ...
    pub async fn rename(
        ...
        match self {
            Self::Memory(store) => Ok(store.rename(visitor_id, chat_id, title)),
            Self::Postgres(store) => store.rename(visitor_id, chat_id, title).await,
        }
    }

    pub async fn delete(&self, visitor_id: &str, chat_id: &str) -> Result<bool, StoreError> {
        match self {
            Self::Memory(store) => Ok(store.delete(visitor_id, chat_id)),
            Self::Postgres(store) => store.delete(visitor_id, chat_id).await,
        }
    }
    ...
    pub async fn set_summary(
        ...
        match self {
            Self::Memory(store) => Ok(store.set_summary(visitor_id, chat_id, summary)),
            Self::Postgres(store) => store.set_summary(visitor_id, chat_id, summary).await,
        }
    }
}
...
pub(crate) fn summary_of(record: HistoryRecord) -> HistorySummary {
    HistorySummary {
...
pub(crate) fn detail_of(record: HistoryRecord) -> HistoryDetail {
    HistoryDetail {
        id: record.chat_id.clone(),
        chat_id: record.chat_id,
        title: record.title,
