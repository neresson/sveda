...
use uuid::Uuid;
use veda_index::WorkspaceIndex;
use veda_llm::ToolSpec;
use veda_mcp::{HostMcpClient, McpTool};
use veda_protocol::{StreamEvent, ToolProgressTask};
...
mod tests {
    ...
    fn workspace() -> (std::path::PathBuf, WorkspaceIndex) {
        let nanos = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("veda-agent-index-{nanos}"));
