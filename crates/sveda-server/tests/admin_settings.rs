...
async fn reads_settings_with_admin_key_header() {
    ...
    assert_eq!(payload["web"]["enabled"], true);
    assert_eq!(payload["web"]["searxng_url"], "");
    ...
    assert!(payload.get("embeddings").is_some());
    assert!(payload.get("default_embedding").is_some());
}
...
#[tokio::test]
async fn post_can_add_and_update_embedding_without_replacing_key() {
    let state = admin_state();
    let (status, body) = send(
        state.clone(),
        "POST",
    ...
    assert_eq!(status, StatusCode::OK);
    let payload: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(payload["default_embedding"], "openai-small");
    assert_eq!(payload["embeddings"][0]["id"], "openai-small");
    assert_eq!(payload["embeddings"][0]["protocol"], "openai");
    assert_eq!(payload["embeddings"][0]["dimensions"], 1536);
    assert_eq!(payload["embeddings"][0]["key"], "••••••••");
    ...
    assert_eq!(status, StatusCode::OK);
    let payload: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(payload["embeddings"][0]["label"], "OpenAI small updated");
    assert_eq!(payload["embeddings"][0]["dimensions"], 1024);
    assert_eq!(payload["embeddings"][0]["key"], "••••••••");
}
...
async fn public_embed_config_omits_secrets() {
    ...
    assert!(payload.get("web").is_none());
    assert!(payload.get("embeddings").is_none());
