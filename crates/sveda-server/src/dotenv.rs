...
fn skip_env_file(value: &str) -> bool {
    matches!(value.trim(), "" | "/dev/null" | "none" | "-")
}

fn env_file_candidates() -> Vec<PathBuf> {
    if let Ok(explicit) = std::env::var("SVEDA_ENV_FILE") {
        let trimmed = explicit.trim();
        if skip_env_file(trimmed) {
            return Vec::new();
        }
...
mod tests {
    ...
    #[test]
    fn sveda_env_file_dev_null_skips_runtime_dotenv() {
        assert!(skip_env_file("/dev/null"));
        assert!(skip_env_file("none"));
        assert!(skip_env_file("-"));
        assert!(!skip_env_file("apps/runtime/.env"));
    }
}
