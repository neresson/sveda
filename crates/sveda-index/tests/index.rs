...
use sveda_index::{chunk_lines, cosine, embed_text, prepare};
...
fn unique_dir(tag: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("sveda-index-{tag}-{nanos}"));
...
fn grep_finds_source_hits_with_line_numbers() {
    let root = write_workspace();
    let index = prepare(&root);
    let hits = index.grep("hello from sveda", 8);
    assert_eq!(hits.len(), 1);
    assert_eq!(hits[0].path, "src/hello.rs");
    assert_eq!(hits[0].line, 2);
    assert!(hits[0].text.contains("hello from sveda"));
    let _ = fs::remove_dir_all(root);
}
...
#[test]
fn prepare_hashed_uses_requested_dimensions() {
    let root = write_workspace();
    let index = sveda_index::prepare_hashed(&root, 1536);
    assert_eq!(index.dimensions(), 1536);
    assert!(index.embedding_id().is_none());
    let hits = index.search("occupancy markdown", 5);
    assert!(!hits.is_empty());
    let _ = fs::remove_dir_all(root);
}
...
#[test]
fn finish_rejects_dimension_mismatch() {
    let root = write_workspace();
    let collected = sveda_index::collect(&root);
    let count = collected.embed_sources().len();
    let mut embeddings = vec![vec![0.1; 1536]; count];
    if let Some(first) = embeddings.first_mut() {
        *first = vec![0.1; 256];
    }
    assert!(collected.finish(embeddings, Some("openai".into())).is_err());
    let _ = fs::remove_dir_all(root);
}
