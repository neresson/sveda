...
pub use history::{
    chat_to_json, conversation_json, denied_tool_output, display_messages, json_to_chat,
    pending_tool_calls, preview_from, split_prompt_and_history, PendingToolCall,
};
pub use title::{
    derive_provisional_title, generate_title, is_placeholder_title, sanitize_generated_title,
};
pub use tools::{
    normalize_output, resource_links, MemoryBackend, NativeHandler, ToolRuntime,
    READ_CODE_TOOL_NAME, SEARCH_CODE_TOOL_NAME, SEARCH_TOOL_NAME, SPAWN_TOOL_NAME,
};
...
pub struct AgentRequest {
    ...
    pub confirming_client_tools: Vec<String>,
    ...
    stream! {
        ...
        for step in 0..max_steps {
            ...
            while let Some(item) = llm_stream.next().await {
                match item {
                    ...
                    Ok(LlmChunk::ToolCall { id, name, input }) => {
                        ...
                        let confirmation = if tool_needs_confirmation(&request, &runtime, &name) {
            ...
            if finish_reason == "tool_calls" {
                ...
                let waiting_confirmation = tool_calls
                    .iter()
                    .any(|(_, name, _)| tool_needs_confirmation(&request, &runtime, name));
                let executable: Vec<(String, String, serde_json::Value)> = tool_calls
                    .iter()
                    .filter(|(_, name, _)| {
                        runtime.known(name) && !runtime.requires_confirmation(name)
                    })
                    .cloned()
                    .collect();
...
pub fn confirming_client_tool_names(client_tools: Option<&Vec<serde_json::Value>>) -> Vec<String> {
    client_tools
        .map(|tools| {
            tools
                .iter()
...
fn tool_needs_confirmation(request: &AgentRequest, runtime: &ToolRuntime, name: &str) -> bool {
    runtime.requires_confirmation(name)
        || request
            .confirming_client_tools
            .iter()
            .any(|tool| tool == name)
}
...
mod tests {
    ...
    use serde_json::json;
    use sveda_llm::{LlmClient, LlmError, ScriptedClient, StoredToolCall};
    ...
    fn request() -> AgentRequest {
        AgentRequest {
            ...
            confirming_client_tools: Vec::new(),
    ...
    #[tokio::test]
    async fn confirmation_tool_waits_while_other_tools_still_run() {
        let llm = ScriptedClient::queue(vec![vec![
            Ok(LlmChunk::ToolCall {
                id: "call-search".into(),
        ...
        let backend = MemoryBackend::new(
            [
                ...
                ("delete_post".into(), json!({ "success": true })),
            ]
            .into_iter()
            .collect(),
        );
        ...
        let runtime = ToolRuntime::memory(
            ...
            vec![
                ...
                sveda_mcp::McpTool {
                    name: "delete_post".into(),
                    description: "Delete a post".into(),
                    input_schema: json!({ "type": "object", "properties": {} }),
                    domain: "posts".into(),
                    mode: "delete".into(),
        ...
        let executed = calls.lock().expect("calls");
        assert_eq!(executed.len(), 1);
        assert_eq!(executed[0].0, "search_posts");
        let delete_call = events.iter().find(|event| {
            matches!(
                event,
                StreamEvent::ToolCall { tool_name, .. } if tool_name == "delete_post"
            )
        });
        match delete_call {
            Some(StreamEvent::ToolCall {
                ...
                assert_eq!(*target, ToolTarget::Backend);
            }
            other => panic!("expected delete tool.call, got {other:?}"),
        }
        assert!(!events.iter().any(|event| {
            matches!(
                event,
                StreamEvent::ToolResult { tool_name, .. } if tool_name == "delete_post"
            )
        }));
        assert!(events.iter().any(|event| {
            matches!(
                event,
                StreamEvent::ToolResult { tool_name, .. } if tool_name == "search_posts"
            )
        }));
    }

    #[test]
    fn pending_tool_calls_keep_unresolved_arguments() {
        let pending = pending_tool_calls(&[
            json!({
                "role": "assistant",
                "tool_calls": [{
                    "id": "call-delete",
                    "function": { "name": "delete_post", "arguments": "{\"id\":\"1\"}" }
                }]
            }),
            json!({
                "role": "tool",
        ...
        assert_eq!(pending[0].id, "call-delete");
        assert_eq!(pending[0].arguments["id"], "1");
    }
    ...
    fn calendar_tool() -> sveda_mcp::McpTool {
        sveda_mcp::McpTool {
    ...
    #[tokio::test]
    async fn llm_error_after_tools_does_not_emit_max_steps() {
        let llm = ScriptedClient::queue(vec![
            vec![
                Ok(LlmChunk::ReasoningDelta("plan".into())),
                Ok(LlmChunk::ToolCall {
            ...
            vec![Err(LlmError::Other(
                r#"{"error":{"message":"The `reasoning_text` in the thinking mode must be passed back to the API."}}"#
                    .into(),
            ))],
        ]);
        let runtime = ToolRuntime::memory(
        ...
        assert!(!types.contains(&"max_steps"));
    }
    ...
    #[test]
    fn split_expands_tool_results_on_assistant_parts() {
        let (prompt, history) = split_prompt_and_history(
            Some("try again"),
            &[
                json!({"role": "user", "content": "search water"}),
                json!({
                    "role": "assistant",
                    "content": "searching",
                    "parts": [
        ...
        assert_eq!(history[1].reasoning, "need tools");
        assert_eq!(history[1].tool_call_id, None);
        assert_eq!(history[1].tool_calls[0].id, "call_01_abc");
        assert_eq!(history[2].role, "tool");
        assert_eq!(history[2].tool_call_id.as_deref(), Some("call_01_abc"));
        assert!(history[2].content.contains("ok"));
    }

    #[test]
    fn conversation_json_keeps_reasoning_and_tool_loop() {
        let items = conversation_json(
            &[],
            "search water",
            "",
            "need tools",
            &[StoredToolCall {
                ...
                arguments: "{\"q\":\"water\"}".into(),
            }],
            &[("c1".into(), "ok".into())],
        );
        assert_eq!(items[0]["role"], "user");
        assert_eq!(items[1]["role"], "assistant");
        assert_eq!(items[1]["reasoning"], "need tools");
        assert_eq!(items[1]["tool_calls"][0]["id"], "c1");
        assert_eq!(items[2]["role"], "tool");
        assert_eq!(items[2]["tool_call_id"], "c1");
    }

    #[test]
    fn display_messages_keep_reasoning_and_tool_parts() {
        let messages = display_messages(
            ...
            &[("c1".into(), "web_search".into(), "{\"ok\":true}".into())],
            &[],
        );
        ...
        assert_eq!(parts[3]["type"], "tool-result");
    }
}
