...
impl std::fmt::Display for UnknownModelError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.id.is_empty() {
            write!(f, "Unknown Veda model.")
        } else {
            write!(f, "Unknown Veda model [{}].", self.id)
        }
    }
}
...
impl Catalog {
    ...
    pub fn from_env() -> Self {
        let key = std::env::var("DEEPSEEK_API_KEY")
            .or_else(|_| std::env::var("VEDA_DEEPSEEK_API_KEY"))
            .unwrap_or_default();
        let mut catalog = Self::builtin(key);
        if let Ok(default_id) =
            std::env::var("VEDA_DEFAULT_MODEL").or_else(|_| std::env::var("VEDA_MODEL"))
        {
        ...
        if let Ok(failover) = std::env::var("VEDA_FAILOVER") {
