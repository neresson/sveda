...
export const SvedaI18nContext = createContext<SvedaI18n>();
...
export function createSvedaI18n(options: SvedaI18nOptions = {}): SvedaI18n {
  ...
  return {
    ...
    extend(loc: string, localeMessages: SvedaMessages): void {
      messages[loc] = { ...(messages[loc] ?? {}), ...localeMessages };
    },
  };
}

export function useSvedaT(): (key: string, params?: Record<string, unknown>) => string {
  const i18n = useContext(SvedaI18nContext);
  if (!i18n) {
