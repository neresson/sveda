import type { SvedaChatSession, SvedaClient } from '@sveda-ai/core';
import svedaBaseStyles from '@sveda-ai/vue/styles.css?inline';
import {
  createSveda,
  useSvedaChat,
  useSvedaClient,
...
export type SvedaChatElementConfig = Omit<Partial<SvedaPluginOptions>, 'endpoints'> & {
  endpoints?: Partial<SvedaPluginOptions['endpoints']>;
  styles?: string | false;
};

export type SvedaChatElementTheme = 'light' | 'dark';

export interface SvedaChatElementRootApi {
  open(): void;
  close(): void;
  sendMessage(text: string, context?: Record<string, unknown>): Promise<void>;
  getSession(): SvedaChatSession | null;
  getClient(): SvedaClient;
}

interface SvedaElementState {
  brandName?: string;
  brandLogo?: string;
  pageUrl?: string;
...
type SvedaElementEmit = (type: string, detail?: unknown) => void;
...
function createSvedaElementRoot(state: SvedaElementState, emit: SvedaElementEmit) {
  return defineComponent({
    name: 'SvedaElementRoot',
    setup(_props, { expose }) {
      const store = useSvedaChat();
      const client = useSvedaClient();
      const streaming = useSvedaStreaming(store, () => {}, {});
      ...
      const subscribeSession = (chatId: string): void => {
        ...
        sessionSubscriptions.set(chatId, [
          session.on('messages', () =>
            emit('sveda-message', { chatId, messages: session.messages })
          ),
          session.on('status', payload => emit('sveda-status', { chatId, status: payload })),
          session.on('error', payload =>
            emit('sveda-error', { chatId, error: serializeError(payload) })
          ),
          session.on('toolProgress', payload =>
            emit('sveda-tool-progress', { chatId, event: payload })
          ),
          session.on('finish', payload => emit('sveda-finish', { chatId, event: payload })),
          session.on('title', payload => emit('sveda-title', { chatId, title: payload })),
        ]);
      };
      ...
      expose({
        ...
        getClient: () => client,
      } satisfies SvedaChatElementRootApi);
      ...
      return () =>
        h(SvedaChat, {
          ...
          onNavigate: (url: string) => emit('sveda-navigate', { url }),
          notify: (kind: string, message: string) => emit('sveda-notify', { kind, message }),
        });
    },
  });
}
...
export class SvedaChatElement extends HTMLElement {
  ...
  private rootApi: SvedaChatElementRootApi | null = null;
  ...
  private elementConfig: SvedaChatElementConfig = {};

  private connected = false;

  private readonly state = reactive<SvedaElementState>({});

  private readonly emit: SvedaElementEmit = (type, detail) => {
    this.dispatchEvent(new CustomEvent(type, { detail, bubbles: true, composed: true }));
  };

  get config(): SvedaChatElementConfig {
    return this.elementConfig;
  }

  set config(value: SvedaChatElementConfig | null | undefined) {
  ...
  async sendMessage(text: string, context?: Record<string, unknown>): Promise<void> {
    if (!this.rootApi) {
      throw new Error('[sveda] <sveda-chat> is not mounted. Configure a stream endpoint first.');
    }
  ...
  setStyles(css: string): void {
    ...
    if (!this.styleElement) {
      ...
      this.styleElement.setAttribute('data-sveda-styles', '');
  ...
  private ensureMountPoint(): void {
    ...
    this.mountPoint.setAttribute('data-sveda-root', '');
  ...
  private resolveOptions(): SvedaPluginOptions {
    const config = this.elementConfig;
    ...
    ) as SvedaPluginOptions['endpoints'];

    const protocolModeAttribute = this.getAttribute('protocol-mode');
    const protocolMode =
      protocolModeAttribute === 'vercel' || protocolModeAttribute === 'sveda'
  ...
  private mount(): void {
    ...
    const sveda = createSveda(options);
    const app = createApp(createSvedaElementRoot(this.state, this.emit));
    app.use(sveda);
    this.rootApi = app.mount(this.mountPoint) as unknown as SvedaChatElementRootApi;
    this.app = app;
...
export function defineSvedaChatElement(tag = 'sveda-chat'): void {
  if (typeof customElements === 'undefined') {
    return;
  }
  if (!customElements.get(tag)) {
    customElements.define(tag, SvedaChatElement);
  }
}
