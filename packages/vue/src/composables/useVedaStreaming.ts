...
import type { VedaContextUsageEvent, VedaMaxStepsEvent, VedaToolProgressEvent } from '@veda-ai/protocol';
...
export interface VedaStreamingStore {
  currentChat: Ref<{
    id: string;
    title?: string;
    messages?: VedaDisplayMessage[];
  } | null>;
  setChatMessages: (chatId: string, messages: VedaDisplayMessage[]) => void;
...
export interface VedaStreamingOptions {
  newChatLabel?: () => string;
  resolveSendOptions?: () => Pick<VedaSendOptions, 'model' | 'provider' | 'options'>;
  onMaxStepsReached?: (chatId: string, data: { maxSteps: number }) => void;
  onToolProgress?: (chatId: string, event: VedaToolProgressEvent) => void;
  onContextUsage?: (chatId: string, event: VedaContextUsageEvent) => void;
  onError?: (chatId: string, error: unknown) => void;
  onDone?: (chatId: string) => void;
}

export type VedaSendPayload =
  | string
  | {
      displayText: string;
      promptText: string;
...
export function useVedaStreaming(
  store: VedaStreamingStore,
  scrollToBottom: (options?: { behavior?: string; onlyIfNearBottom?: boolean }) => void,
  options: VedaStreamingOptions = {}
) {
  const t = useVedaT();
  const client = inject(VedaClientKey, null) as VedaClient | null;
  if (!client) {
    throw new Error('[veda] useVedaStreaming requires the Veda plugin to be installed.');
  }
  ...
  const statusByChatId = ref<Record<string, VedaSessionStatus>>({});
  ...
  const setChatStatus = (chatId: string, status: VedaSessionStatus) => {
  ...
  const applyDisplayOverrides = (
    ...
    return messages.map(message => {
      ...
      const parts: VedaMessagePart[] = [
        ...(override ? [{ type: 'text', text: override } as VedaMessagePart] : []),
        ...fileParts,
      ];
  ...
  const ensureSession = (chatId: string): VedaChatSession => {
    ...
    const offStatus = session.on('status', payload => {
      setChatStatus(chatId, payload as VedaSessionStatus);
    });
    ...
    const offMaxSteps = session.on('maxSteps', payload => {
      const event = payload as VedaMaxStepsEvent | undefined;
    ...
    const offContextUsage = session.on('contextUsage', payload => {
      const event = payload as VedaContextUsageEvent | undefined;
    ...
    const offToolProgress = session.on('toolProgress', payload => {
      options.onToolProgress?.(chatId, payload as VedaToolProgressEvent);
    });
  ...
  const resolveSendOptions = (): VedaSendOptions => ({
    ...(options.resolveSendOptions?.() ?? {}),
  });

  const sendMessage = async (
    userPayload: VedaSendPayload,
    ...
    const sendOptions: VedaSendOptions = {
      ...resolveSendOptions(),
      context: { ...pageContext },
