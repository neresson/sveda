...
export type VedaAgentTasksState = {
  runId: string | null;
  phase: string;
  tasks: VedaAgentTaskItem[];
};

export type VedaAgentTasksPayload = {
  runId?: string;
  phase?: string;
  tasks?: Array<{
    id?: unknown;
...
const emptyState = (): VedaAgentTasksState => ({
  runId: null,
  phase: '',
  tasks: [],
});

export function useVedaAgentTasks(
  options: {
    subscribe?: (
  ...
  const state = ref<VedaAgentTasksState>(emptyState());

  const hasActiveTasks = computed(() =>
    state.value.tasks.some(task => task.status === 'running' || task.status === 'pending')
  );

  const mergePayload = (payload: VedaAgentTasksPayload) => {
