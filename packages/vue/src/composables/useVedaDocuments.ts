...
export type VedaExtractedDocumentItem = {
  filename?: string;
  ok?: boolean;
  text?: string;
  truncated?: boolean;
...
export function useVedaDocuments(
  options: {
    endpoint?: string;
  ...
  const buildChatDocumentSections = (items: VedaExtractedDocumentItem[]): string => {
    const rows: string[] = [];
  ...
  const extractChatDocuments = async (
    rawFiles: File[]
  ): Promise<{ items: VedaExtractedDocumentItem[] }> => {
    ...
    const payload = (await response.json().catch(() => ({}))) as {
      ...
      items?: VedaExtractedDocumentItem[];
    };
