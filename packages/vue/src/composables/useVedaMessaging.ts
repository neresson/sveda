...
import type { VedaExtractedDocumentItem } from './useVedaDocuments';
...
export function useVedaMessaging(deps: {
  ...
  extractChatDocuments: (files: File[]) => Promise<{ items: VedaExtractedDocumentItem[] }>;
  buildChatDocumentSections: (items: VedaExtractedDocumentItem[]) => string;
