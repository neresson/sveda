...
export function useSvedaMessaging(deps: {
  ...
  confirmationPending?: Ref<boolean>;
  ...
  const sendMessage = async () => {
    if (deps.isLoading.value || deps.confirmationPending?.value || !deps.currentChat.value) {
      return;
    }
  ...
  const sendQuickPrompt = async (prompt: string) => {
    if (!prompt || deps.isLoading.value || deps.confirmationPending?.value || !deps.currentChat.value) {
      return;
    }
