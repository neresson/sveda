<script setup>
  ...
  const composerDisabled = computed(() => props.isLoading || props.confirmationPending);

  const voiceInputDisabled = computed(() => composerDisabled.value || !props.hasCurrentChat || props.isStreaming);

  const attachmentToolbarDisabled = computed(() => composerDisabled.value || !props.hasCurrentChat || props.isStreaming || isListening.value);
