<script setup>
  ...
  const emit = defineEmits(['activity-link-click', 'confirm-tool', 'deny-tool']);
  ...
  const needsConfirmation = inv => inv?.confirmation === 'required' && !isToolInvocationComplete(inv);

  const getToolInvocationStatus = inv => {
    if (needsConfirmation(inv)) {
      return 'confirmation';
    }
    ...
    if (props.invocations.length > 0) {
      return props.invocations.map((inv, index) => {
        ...
        return {
          ...
          status,
          detail,
          needsConfirmation: needsConfirmation(inv),
      ...
      steps.forEach(step => {
        if (step.kind === 'tool') {
          if (step.status === 'running') {
            ...
            const previous = prevMap.get(step.id);
            const wasOpen = previous?.status === 'running' || previous?.status === 'confirmation' || previous?.needsConfirmation;
            if (wasOpen) {
    ...
    >
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              <component
                v-else-if="!step.needsConfirmation && (step.detail || step.kind === 'tool' || step.kind === 'subagent')"
                :is="isStepExpanded(step) ? ChevronDown : ChevronRight"
                class="ml-auto h-3 w-3 text-muted-foreground"
              />
            </button>
            <div
              v-if="isStepExpanded(step) && !step.needsConfirmation && !hasDisplayLink(step) && (step.detail || step.kind === 'tool' || step.kind === 'subagent')"
            ...
            <div
              v-if="step.needsConfirmation"
              class="mt-1 border border-border bg-card px-2.5 py-2"
            >
              <p class="text-xs leading-snug text-foreground">{{ t('toolConfirmationPrompt') }}</p>
              <pre v-if="step.detail" class="mt-1.5 max-h-24 overflow-auto whitespace-pre-wrap break-words font-mono text-[11px] leading-snug text-muted-foreground">{{ step.detail }}</pre>
              <div class="mt-2 flex items-center gap-2">
                <button
                  type="button"
                  class="bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground"
                  @click.stop="emit('confirm-tool', step.toolCallId)"
                >
                  {{ t('toolConfirm') }}
                </button>
                <button
                  type="button"
                  class="border border-border bg-card px-3 py-1.5 text-xs font-medium text-foreground"
                  @click.stop="emit('deny-tool', step.toolCallId)"
                >
                  {{ t('toolDeny') }}
                </button>
              </div>
      ...
      <div v-else>
        ...
        >
          ...
          />
          <component
            v-else-if="!item.step.needsConfirmation && (item.step.detail || item.step.kind === 'tool' || item.step.kind === 'subagent')"
            :is="isStepExpanded(item.step) ? ChevronDown : ChevronRight"
            class="ml-auto h-3 w-3 text-muted-foreground"
          />
        </button>
        <div
          v-if="isStepExpanded(item.step) && !item.step.needsConfirmation && !hasDisplayLink(item.step) && (item.step.detail || item.step.kind === 'tool' || item.step.kind === 'subagent')"
          class="pb-1 pl-4 text-muted-foreground/80"
        >
          <AnimatedStreamBlock
        ...
        <div
          v-if="item.step.needsConfirmation"
          class="mt-1 border border-border bg-card px-2.5 py-2"
        >
          <p class="text-xs leading-snug text-foreground">{{ t('toolConfirmationPrompt') }}</p>
          <pre v-if="item.step.detail" class="mt-1.5 max-h-24 overflow-auto whitespace-pre-wrap break-words font-mono text-[11px] leading-snug text-muted-foreground">{{ item.step.detail }}</pre>
          <div class="mt-2 flex items-center gap-2">
            <button
              type="button"
              class="bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground"
              @click.stop="emit('confirm-tool', item.step.toolCallId)"
            >
              {{ t('toolConfirm') }}
            </button>
            <button
              type="button"
              class="border border-border bg-card px-3 py-1.5 text-xs font-medium text-foreground"
              @click.stop="emit('deny-tool', item.step.toolCallId)"
            >
              {{ t('toolDeny') }}
            </button>
          </div>
