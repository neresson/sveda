...
Internet search (`web_search` / `web_fetch`) is on by default for the public embed. The sidecar searches DuckDuckGo then Bing itself. Operators can disable it or point at a self-hosted SearXNG in **Admin → Runtime**. `SVEDA_SEARXNG_URL` only seeds the first boot.
